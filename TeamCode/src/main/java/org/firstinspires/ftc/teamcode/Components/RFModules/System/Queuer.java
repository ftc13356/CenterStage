...
/**
 * Warren Zhou 8/23 function scheduler that allows for sequential, asynchronous, and other forms of
 * scheduling
 */
public class Queuer {
  private final ArrayList<QueueElement> queueElements;
  private boolean firstLoop = true, mustFinish = false;
  private int currentlyQueueing = 0,
      currentEvent = -1,
  ...
  /**
   * creates new queue element if first loop updates which element is currently being queued and
  ...
  /**
   * Set currently queued synchronous event to done, manually skipping it, overriding any other
   * conditions logs which event gets finished at general surface level
   */
  public void done() {
    int inde = 908;
   ...
   * which event gets updated at general fine level
   *
   * @param p_ind index of queueElement in question
   */
  public void updateStartConditions(int p_ind) {
    if (p_ind < queueElements.size()) {
      if (queueElements.get(p_ind).startCondition
          != recalcStartPosSkipOptional(
              p_ind,
              queueElements.get(p_ind).isAsynchronous(),
              queueElements.get(p_ind).isOptional())) {
        queueElements
            .get(p_ind)
            .setStartCondition(
                recalcStartPosSkipOptional(
        ...
        LOGGER.log(
            "Event: " + p_ind + " StartCondition: " + queueElements.get(p_ind).startCondition);
  ...
  /**
   * return if the currently queued event +1 is started, useful as a time saving quick done
   * condition
   */
  public boolean isStarted() {
    if (currentlyQueueing + 1 < queueElements.size()) {
      return queueElements.get(currentlyQueueing + 1).isStarted()
          || queueElements.get(currentlyQueueing + 1).isDone();
  ...
  public boolean queue(
    ...
    else if (!queueElements.get(currentlyQueueing).isMustFinish()
        && !queueElements.get(currentlyQueueing).isShouldFinish()) {
      // calculate if event should run
      isReady = queueElements.get(currentlyQueueing).isReady(currentEvent, p_extra_condition);
    ...
    // if current event must wait for all previous events to finsih
      ...
      // calculate if event should run
      isReady =
          queueElements.get(currentlyQueueing).isReady(completeCurrentEvent, p_extra_condition);
    }
...
//        queueElements.get(currentlyQueueing).setStarted(true);
  ...
  /**
   * set the current event to whatever is currently being queued, skip back or forward into the
   * queue logs that this function is called event gets updated at general surface level
   */
  ...
  /**
   * reset the queuer to factory settings logs that this function is being called to general surface
   * level
   */
  public void reset() {
    LOGGER.setLogLevel(RFLogger.Severity.INFO);
  ...
  private int recalcStartPosSkipOptional(int p_ind, boolean p_asynchronous, boolean p_Optional) {
    ...
    for (int i = 0; i < p_ind; i++) {
      if ((!queueElements.get(p_ind - i - 1).isAsynchronous()
              || queueElements.get(p_ind - i - 1).isMustFinish())
          && (!queueElements.get(p_ind - i - 1).isOptional()
              || queueElements.get(p_ind - i - 1).getSkipOption())) {
  ...
  /**
   * create new queueElement logs which event was queued
   *
  ...
  private void createQueueElement(boolean p_asynchrnous, boolean p_isOptional) {
    ...
    if (!mustFinish) {
      ...
      p_asynchrnous = false;
      queueElements.add(
          new QueueElement(queueElements.size(), p_asynchrnous, startCondition, true));
      LOGGER.log("event# : " + (queueElements.size() - 1) + ", StartCondition : " + startCondition);
    }
  }

  /**
   * update which element is currently being queued(processed) and which element is currently being
   * executed log which element is currently being queued and which is being executed
   *
   * @param p_done_condition is the function done
   * @param p_isOptional is the function optional
   */
  private void updateQueuer(boolean p_done_condition, boolean p_isOptional) {
    ...
    if (!firstLoop) {
      if (queueElements.get(currentlyQueueing).isStarted()
          && !queueElements.get(currentlyQueueing).isDone()) {
        ...
        if (p_done_condition) {
          ...
          LOGGER.setLogLevel(RFLogger.Severity.INFO);
          LOGGER.log(
              "currently Queueing event# : "
                  + currentlyQueueing
                  + "is Done, "
                  + "completeEvents"
                  + completeCurrentEvent);
          if (currentlyQueueing > currentEvent
              && !queueElements.get(currentlyQueueing).isAsynchronous()) {
            currentEvent = currentlyQueueing;
            LOGGER.log(
                "currenty finished event# : "
                    + currentEvent
                    + "is Done, "
                    + "completeEvents"
                    + completeCurrentEvent);
