...
public class IDRobot extends BasicRobot {
    ...
    public static double FOR_CONST = 4.00, FOR_MULT = 1, SIDE_CONST = 2, SIDE_MULT = 0.8, MOVE_INTERVAL = 0.5, DELAY_TIME = 0.3, DROP_DELAY_TIME = 0.12, MIN_EXT = 5.8, HANGEXT1 = 20.5, HANGROT1 = 110,
...
//                    }
    ...
    public void teleOp(boolean isBlue) {
        ...
        if (follower.isTeleDrive() || (abs(op.gamepad1.left_stick_y) > 0.001 || abs(op.gamepad1.left_stick_x) > 0.001 || abs(op.gamepad1.right_stick_x) > 0.001)) {
            ...
            follower.rawDriving(new Pose2d(-op.gamepad1.left_stick_y * driveConst * 1 / .7, op.gamepad1.left_stick_x * max(driveConst*.8/.7, 0.4), -op.gamepad1.right_stick_x * driveConst), arm.getExt());
//            isAutoGrab = false;
//            targeted = false;
        }
