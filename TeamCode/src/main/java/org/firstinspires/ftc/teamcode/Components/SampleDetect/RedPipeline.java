...
public class RedPipeline extends OpenCvPipeline {
    ...
    public static double UH = 10, LH = 160, S = 100, V = 100;
    Scalar upperBlue = new Scalar(35, 255, 255);
    ...
    public static double AREA_THRESH = .85, BYPASS_RATIO = 0.1, FCL = 1, EPSILON = 0.04, UP_TOLERANCE = 0.5, DOWN_TOLERANCE = 0.5, CLASSUP_TOL = 0.8, CLASSDOWN_TOL = 0.7, PLUS_ULTRA = 0.5, UP_TOLERANCE2 = 2, DOWN_TOLERANCE2 = 1.25, FOUR_THRESH = 0.9;
    ...
    public Mat processFrame(Mat input) {
        ...
        Scalar lowerBlue = new Scalar(0, S, V);
        Scalar upperBlue = new Scalar(UH, 255, 255);

        Scalar lowerRed = new Scalar(LH, S, V);
...
//        input.release();
    ...
    public Point[] order4(Point[] pts) {
        Point center = new Point(640, 360);
        double[] distance = {distance(center, pts[0]), distance(center, pts[1]), distance(center, pts[2]), distance(center, pts[3])};
        int min = 0;
        for (int i = 1; i < 3; i++) {
        ...
        double[] angles = calculateQuadrilateralAngles(pts);
        int opp = min + 2;
        if (opp > 3)
            opp -= 4;
        if (angles[min] / angles[opp] < FOUR_THRESH) {
            int p1 = min + 1, p2 = min - 1;
            if (p1 == 4)
                p1 = 0;
            if (p2 == -1)
                p2 = 3;
            if (distance(pts[min], pts[p1]) > distance(pts[min], pts[p2])) {
                int p3 = p1 + 1;
                if (p3 > 3)
                    p3 = 0;
                pts = new Point[]{pts[min], pts[p1], pts[p3], pts[p2]};
            } else {
                int p3 = p2 - 1;
                if (p2 < 0)
                    p3 = 4;
                pts = new Point[]{pts[min], pts[p2], pts[p3], pts[p1]};
            }
        }
        return pts;
    }
    ...
    // Function to calculate the four angles of a quadrilateral given an array of points
    public double[] calculateQuadrilateralAngles(Point[] points) {
        // Ensure there are exactly 4 points in the array
        if (points.length != 4) {
            throw new IllegalArgumentException("The input must contain exactly 4 points.");
        }


        // Array to store the four angles
        double[] angles = new double[4];

        // Vector from points[0] to points[1] and from points[0] to points[3]
        Point P0P1 = new Point(points[1].x - points[0].x, points[1].y - points[0].y);
        Point P0P3 = new Point(points[3].x - points[0].x, points[3].y - points[0].y);
        angles[0] = calculateAngle(P0P1, P0P3);

        // Vector from points[1] to points[2] and from points[1] to points[0]
        Point P1P2 = new Point(points[2].x - points[1].x, points[2].y - points[1].y);
        Point P1P0 = new Point(points[0].x - points[1].x, points[0].y - points[1].y);
        angles[1] = calculateAngle(P1P2, P1P0);

        // Vector from points[2] to points[3] and from points[2] to points[1]
        Point P2P3 = new Point(points[3].x - points[2].x, points[3].y - points[2].y);
        Point P2P1 = new Point(points[1].x - points[2].x, points[1].y - points[2].y);
        angles[2] = calculateAngle(P2P3, P2P1);

        // Vector from points[3] to points[0] and from points[3] to points[2]
        Point P3P0 = new Point(points[0].x - points[3].x, points[0].y - points[3].y);
        Point P3P2 = new Point(points[2].x - points[3].x, points[2].y - points[3].y);
        angles[3] = calculateAngle(P3P0, P3P2);
